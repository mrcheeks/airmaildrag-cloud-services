...
export default async function handler({ req, res, log, error }: {
  ...
  try {
    ...
    const guests = (guestsResponse.documents || []) as unknown as GuestUser[];
    ...
    const guestsByTournament = new Map<string, GuestUser[]>();
    ...
    for (const [tournamentId, tournamentGuests] of guestsByTournament) {
      ...
      try {
        ...
        log(`Tournament ${tournamentId} (${tournament.name || 'unnamed'}): status=${tournament.status}, created=${tournament.$createdAt}, isCompleted=${isCompleted}, isOldEnough=${isOldEnough}`);
 ...
 */
async function ensureVisitorUser(
  databases: Databases,
  databaseId: string,
 ...
 */
async function reassignGuestGames(
  databases: Databases,
  databaseId: string,
  ...
  for (const query of queries) {
    try {
      ...
      for (const game of response.documents) {
        ...
        if ((game as any).player1 === guestUserId) {
          updates.player1 = VISITOR_USER_ID;
        }
        if ((game as any).player2 === guestUserId) {
          updates.player2 = VISITOR_USER_ID;
        }
        if ((game as any).team1_player1 === guestUserId) {
          updates.team1_player1 = VISITOR_USER_ID;
        }
        if ((game as any).team1_player2 === guestUserId) {
          updates.team1_player2 = VISITOR_USER_ID;
        }
        if ((game as any).team2_player1 === guestUserId) {
          updates.team2_player1 = VISITOR_USER_ID;
        }
        if ((game as any).team2_player2 === guestUserId) {
 ...
 */
async function deleteGuestEntries(
  databases: Databases,
  databaseId: string,
  participantsCollection: string,
  tournamentId: string,
  guestUserId: string,
  log: (msg: string) => void
): Promise<number> {
